...
- `packages/social-agent/IDEA.md` dosyası o modülün kanonik tasarım belgesidir — davranış buradan türetilir.
...
`social-agent` paketinde her tamamlanan iterasyon sonunda şu adımları çalıştır:

```bash
# 1. Paket-prefixed SemVer tag (MAJOR.MINOR.PATCH — kırıcı değişiklik / yeni özellik / düzeltme)
cd packages/social-agent
gh release create social-agent-vX.Y.Z --title "social-agent-vX.Y.Z — <tek satır özet>" --notes "$(sed -n '/## \[social-agent-vX.Y.Z\]/,/## \[/p' CHANGELOG.md | head -n -1)"

# 2. packages/social-agent/CHANGELOG.md başına yeni giriş ekle (bkz. Keep a Changelog formatı)
```

## CHANGELOG Formatı
`packages/social-agent/CHANGELOG.md` dosyasını [Keep a Changelog](https://keepachangelog.com) standardında tut:
- `### Added` / `### Changed` / `### Fixed` / `### Removed` başlıklarını kullan.
- Her release girişi `## [social-agent-vX.Y.Z] — YYYY-MM-DD` satırıyla açılır.
