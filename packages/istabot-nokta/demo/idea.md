https://stitch.withgoogle.com/projects/10788380314520523062
...
```
┌──────────────────────────────────────────────────┐
│         USER INTERFACE (Angular / React / Web)   │
│  Chat Interface + MRLC Progress Bar + Artifacts  │
└──────────────────────────────────────────────────┘
                        ↓
┌──────────────────────────────────────────────────┐
│         istabot-nokta (npm package)              │
│  Zero-dependency, browser + Node.js uyumlu       │
│  Native fetch → Claude API                       │
│                                                  │
│  discover(input, options) → { text, pico, rq }   │
│  design(input, options)   → { text, sampleSize } │
│  execute(input, options)  → { text, testName }   │
│  publish(input, options)  → { text, manuscript } │
│  pipeline(input, options) → tüm fazlar zinciri   │
└──────────────────────────────────────────────────┘
                        ↓
┌──────────────────────────────────────────────────┐
│              Claude API (Anthropic)              │
│  Model: claude-sonnet-4-6 (default)              │
│  Her faz için domain-specific system prompt      │
└──────────────────────────────────────────────────┘
```

**Kullanım (developer ne yaparsa yapsın):**
```js
import { discover } from 'istabot-nokta';
...
// result.text  → LLM'in tam yanıtı
// result.pico  → { P, I, C, O }
// result.rq    → "Tip 2 diyabetli hastalarda..."

// Developer ne yaparsa:
setState(result);           // React
this.idea = result;         // Angular
res.json(result);           // Express API
fs.writeFileSync(...)       // Node.js CLI
```
